...
// ==========================================
// + Settings Modal trigger2 + close button +
// ==========================================
...
// Ctrl + Alt + G to open facebook.com
document.addEventListener("keyup", function (facebook) {
  if (facebook.key == "g" && facebook.altKey && facebook.ctrlKey) {
...
// Ctrl + Alt + R to open yahoo.com
document.addEventListener("keyup", function (yahoo) {
  if (yahoo.key == "r" && yahoo.altKey && yahoo.ctrlKey) {
    window.location.replace("https://yahoo.com");
  }
});

// Ctrl + Alt + P to open bing.com
document.addEventListener("keyup", function (bing) {
  if (bing.key == "p" && bing.altKey && bing.ctrlKey) {
...
document.querySelector("#save-image").addEventListener("click", function () {
  ...
  if (imageUrlValue == "") {
    processingBg.innerHTML =
      '<i class="las la-exclamation-triangle"></i>' + " La URL no puede estar vacía.";
  } else {
    processingBg.innerHTML =
      '<i class="las la-check-circle"></i>' + " Fondo aplicado... ";
...
input.addEventListener("change", (event) => {
  ...
  if (imageSize >= 5) {
    processingBg.innerHTML =
      ...
      " Error, esta imagen tiene un tamaño superior a 5 MB. ";
    +"</span>";
    return;
  }
...
document.querySelector("#delete_custom_image").addEventListener("click", () => {
  ...
  if (!imageUrl && !imageUpload) {
    document.querySelector("#delete_custom_image").innerHTML =
      '<i class="las la-exclamation-circle"></i> <span>Fondo <br/> no encontrado </span>';
    return;
  }

  if (confirm("¿Eliminar fondo personalizado?")) {
    localStorage.removeItem("image_url");
    localStorage.removeItem("imageupload");
    document.querySelector("body").style.backgroundImage = "";
    ...
    document.querySelector("#delete_custom_image").innerHTML =
      '<i class="las la-check-circle"></i> <span>Fondo <br/> eliminado </span>';
  }
});
...
function reset_data() {
  if (confirm("⚠ ¡Espera! ¿Estás seguro de que quieres restablecer todos tus datos?")) {
    document.getElementById("reset_button").innerHTML =
      "<img style='padding-bottom:10px;' class='loading-svg' src='assets/img/loading-red.svg'> <span>Restableciendo...</span><span>¡Espere por favor!</span>";
    localStorage.clear();
    setTimeout(function () {
      location.reload();
    }, 5000);
...
// UI Tweaks: Force header to use white color.
document
